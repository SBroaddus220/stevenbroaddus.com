...
def get_stem_top_center(height=1, x_offset=0, y_offset=0, curve_factor=0.2):
    # Highest point on the z-axis
    z_top = height

    # Calculate the curvature at the top of the stem
...
def plot_roses_and_stems(stem_specs):
    ...
    for spec in stem_specs:
        ...
        generate_stem(height, radius, x_offset, y_offset, z_offset+1.5)
        # Calculate the top center of the stem
        x_top, y_top, z_top = get_stem_top_center(height, x_offset, y_offset)
        # Plot the rose at the top center of the stem
        X, Y, Z = meshgrid_transforms(x_top, y_top, z_top)
...
plot_roses_and_stems(stem_specs)
